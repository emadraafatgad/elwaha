...
class ContainerRequest(models.Model):
     ...
     partner_id = fields.Many2one('res.partner',ondelete='restrict',string='Shipment Company')
     customer = fields.Many2one('res.partner',ondelete='restrict',string='Customer')
     request_date = fields.Datetime('Request Date', readonly=True,default=fields.Datetime.now)
     delivery_date = fields.Date('Delivery Date')
     from_port = fields.Many2one('container.port',ondelete='restrict', string='From Port')
     to_port = fields.Many2one('container.port',ondelete='restrict', string='To Port')
