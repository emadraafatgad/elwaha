...
class AccountAbstractPayment(models.AbstractModel):
    ...
    def _check_deduction_amount(self):
        if self.payment_difference_handling == 'reconcile_multi_deduct':
            if len(self.deduction_ids)>1:
                raise models.ValidationError('  Must be one line  ')
            if  sum(self.deduction_ids.mapped('amount')) > self.payment_difference :
                raise models.ValidationError('  total discount should be less than payment difference ')
...
class AccountPaymentDeduction(models.Model):
    ...
    amount_percentage= fields.Selection(
        string='Select',
        selection=[('a', 'Amount'),
                   ('p', 'Percentage'), ],default='a',
        required=True, )
    ...
    def amount_changed(self):
        if self.amount:
            self.percentage = (self.amount * 100) / self.payment_id.payment_difference

    @api.onchange('percentage')
    def percentage_changed(self):
        if self.percentage:
            self.amount=(self.payment_id.payment_difference*self.percentage)/100
