	        ...
	        if(event){
		        ...
		        if(current_screen === "products"){
		        	...
		        	if(order.get_selected_orderline()){
		        		...
		        		if(keytostring === qty){
							$(self.pos.gui.screen_instances.products.numpad.$el[0]).find("button[data-mode='quantity']").trigger('click');
						}
	//					// select Disc Mode of numpad
						if(keytostring === discount){
							$(self.pos.gui.screen_instances.products.numpad.$el[0]).find("button[data-mode='discount']").trigger('click');
						}
						// select Price Mode of numpad
						if(keytostring === price){
							$(self.pos.gui.screen_instances.products.numpad.$el[0]).find("button[data-mode='price']").trigger('click');
						}
						// payment screen
	...
	////					 Trigger Numpad Numeric Click
	                    if(self.pos.config.enable_keyboard_shortcut && !self.pos.config.is_scan_product){
	                        if($.isNumeric(keytostring) || keytostring === '.' || keytostring === ','){
	                            ...
	                            $(self.pos.gui.screen_instances.products.numpad.$el[0]).find(".number-char:contains("+ keytostring +")").click();
	                        }
	                    }
						// BackSpace of Numpad
						if(event.keyCode === $.ui.keyCode.BACKSPACE){
							$(self.pos.gui.screen_instances.products.numpad.$el[0]).find(".input-button.numpad-backspace").trigger('click');
						}
		        	}
